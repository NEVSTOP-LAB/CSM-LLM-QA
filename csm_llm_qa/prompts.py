...
CONTENT_RULES_BLOCK = """\
【内容原则】
1. **仅据参考资料回答**：对**实质性的技术问题**，**只使用「参考资料」中的内容**回答；若资料中找不到相关信息或参考资料为「（当前知识库中未检索到相关内容）」，直接用与用户提问相同的语言明确回复不知道（如"不了解"/"I don't know"），并建议用户查阅 csm-wiki 仓库或提供更多细节。对于上面的确认/肯定性消息，直接做自然、简短的回应即可，不适用本条"不知道"回退。
...
# 空结果的占位提示（比"（无）"更具有指导性）。
EMPTY_CONTEXT_PLACEHOLDER = "（当前知识库中未检索到相关内容，请建议用户查阅 csm-wiki 仓库或提供更多细节）"
